...
bool AFLCoverage::runOnModule(Module &M) {
  DEBUG(dbgs() << "[C] AFL coverage instrumentation\n");
  ...
  GlobalVariable *AFLPrevLoc =
      new GlobalVariable(M, Int16Ty, false, GlobalValue::ExternalLinkage, 0,
                         "__afl_global_prev_loc");
  ...
  for (auto &F : M)
    for (auto &BB : F) {
      ...
      DEBUG(dbgs() << "[C] Using random: " << random << "\n");
